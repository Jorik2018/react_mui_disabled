...
const List = (props) => {
  ...
  useEffect(() => {
    ...
    if (tableContainer) {
      ...
      tableContainer.style.height = (height - header.offsetHeight) + 'px';
      var pvtUi = tableContainer.querySelector('.pvtUi');
      pvtUi.style.width = tableContainer.style.width;
      var pt = pvtUi.querySelectorAll('.pvtUi:not(._) > * > tr > td');
      if (pt.length) {
        ...
        pt = [].filter.call(pt, (e, c) => {
          if (c === 0) {
            ...
            mo.disconnect();
            e.style.width = tableContainer.style.width;
            e.style.overflow = 'auto';
          }
          return c > 0 && c < pt.length - 1;
        }).forEach((e, i) => {
          var tr = document.createElement("tr");
          tr.appendChild(e);
          if (i === 1) {
        ...
        pvtUi.classList.add("_");
      }
    }
  }, [height, width]);

  useEffect(() => {
    dispatch({ type: 'title', title: 'Pivot Table' });
    fetchData(0)
  }, []);
