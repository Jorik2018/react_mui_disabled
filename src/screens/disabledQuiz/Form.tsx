...
export const Form = () => {
  ...
  const [o, { defaultProps, handleChange, validate, set }]:any = useFormState(useState, {
    'houseAccess': '',
    'instructionGrade': '',
    'maritalStatus': '',
    'typeInsurance': '',
  ...
  useEffect(() => {
    if (pid) {
        ...
        db.disabled.get(isNaN(pid) ? 0 : (1 * pid)).then((e:any) => {
          if (e) {
      ...
      try {
        let s:any = localStorage.getItem("setting");
        if (s) {
          s = JSON.parse(s);
          var o2:any = {};
          o2.red = s.red;
